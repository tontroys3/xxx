...
def login_page():
    ...
    st.info("Platform demo untuk live streaming ke berbagai platform seperti YouTube, Facebook, Twitch, dan lainnya.")
    ...
    with col2:
        ...
        st.markdown("---")
        st.success("**Demo Login:** Username: `admin`, Password: `admin123`")
...
def dashboard_page():
    ...
    if user_streams:
        ...
        data = [[s['title'], s['platform'], s['status'], s['created_at']] for s in recent_streams]
...
def gallery_page():
    ...
    if uploaded_file is not None:
        ...
        st.warning("Catatan: Pada versi demo ini, file tidak benar-benar tersimpan di server.")
        ...
        if st.button("Simpan Video", use_container_width=True):
            new_video = {
                ...
                'file_size': getattr(uploaded_file, 'size', 1024000),
                'created_at': datetime.now().isoformat()
            }
...
def streams_page():
    ...
    with st.form("stream_form"):
        ...
        if submit_button and title and platform and stream_key:
            video_id = None
    ...
    if user_streams:
        for stream in user_streams:
            with st.expander(f"📺 {stream['title']} - {stream['platform']} ({stream['status']})"):
                ...
                with col1:
                    if st.button("▶️ Mulai", key=f"start_{stream['id']}"):
                ...
                with col2:
                    if st.button("⏹️ Berhenti", key=f"stop_{stream['id']}"):
                        for i, s in enumerate(st.session_state.streams):
                            if s['id'] == stream['id']:
                ...
                with col3:
                    if st.button("🗑️ Hapus", key=f"delete_stream_{stream['id']}"):
        ...
        if st.form_submit_button("🔄 Update Password", use_container_width=True):
            if current_password and new_password and confirm_password:
                if new_password == confirm_password and len(new_password) >= 8:
                    ...
                    st.error("Password baru minimal 8 karakter dan harus cocok")
            else:
                st.error("Silakan isi semua field")
